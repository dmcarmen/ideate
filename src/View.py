...
class View(ttk.Frame):
    ...
    def __init__(self, parent, controller) -> None:
        ...
        self.tuds_list = ['s', 'h']
        self.tempuds_list = ['K']
        # TODO: tal vez cambiar para usar directamente m, cm... ^exponente
        ...
        createToolTip(var_bt_dic['velocity'], text='Velocity includes columns Vx, Vy and Vz.')
        ...
        pars_bts_tab.grid(row=1, column=1, rowspan=2, padx=(10, 20), pady=(20, 20))
        ...
        radius_lbl = ttk.Label(volume_frame, text="Radius")
        radius_lbl.grid(row=0, column=0, padx=(20, 0), sticky='w')
        ...
        pintensity_lbl = ttk.Label(volume_frame, text="Point Intensity")
        pintensity_lbl.grid(row=2, column=0, padx=(20, 0), sticky='w')
        ...
        sinkpoints_lbl = ttk.Label(volume_frame, text="Sink points")
        sinkpoints_lbl.grid(row=2, column=2, padx=(20, 0), sticky='w')
        ...
        imgres_lbl = ttk.Label(img_spatial_frame, text="Image resolution")
        imgres_lbl.grid(row=0, column=0, padx=(10, 0), sticky='w')
        ...
        # TODO: not saved, there is only one option
        ...
        pxls_lbl = ttk.Label(img_spatial_frame, text="Pixels/imgres")
        pxls_lbl.grid(row=0, column=2, padx=(20, 10), sticky='w')
        ...
        distance_lbl = ttk.Label(img_spatial_frame, text="Source distance")
        distance_lbl.grid(row=2, column=0, padx=(10, 0), sticky='w')
        ...
        unit_lbl = ttk.Label(img_spatial_frame, text="Units of the image")
        unit_lbl.grid(row=2, column=2, padx=(20, 10), sticky='w')
        ...
        nchan_lbl = ttk.Label(img_velocity_frame, text="Number of channels")
        nchan_lbl.grid(row=0, column=0, padx=(10, 0), sticky='w')
        ...
        velres_lbl = ttk.Label(img_velocity_frame, text="Velocity resolution")
        velres_lbl.grid(row=0, column=1, padx=(20, 0), sticky='w')
        ...
        trans_lbl = ttk.Label(mol_data_tab, text="Transition number:")
        trans_lbl.grid(row=0, column=0, padx=(20, 5), pady=(0, 0), sticky='nw')
        ...
        rel_abundance_lbl = ttk.Label(
            mol_data_tab, text="Relative abundance (r):")
        rel_abundance_lbl.grid(row=2, column=0, padx=(
            20, 5), pady=(0, 0), sticky='nw')
        ...
        lamda_text.grid(row=4, column =0, padx=(20, 5), pady=(30,0), sticky = 's')
        hyperlink= HyperlinkManager(lamda_text)
        
        lamda_text.insert(tk.END, "LAMDA data format",hyperlink.add(partial(webbrowser.open,"https://home.strw.leidenuniv.nl/~moldata/molformat.html")))
    ...
    def update_mol(self, datos_mol):
        if 'mol_name' in datos_mol:  # TODO: elegir la molecula en el arbol...
        ...
        if self.chosen_mol_dic is not None:
            ...
            if len(old_mol) == 0 or old_mol != mol:
                ...
                if old_trans < self.spinbox_len:
                    self.trans_val.set(old_trans) # to keep transition if loading TODO: improve this
                else:
                    self.trans_val.set(1)
                self.trans_changed()
